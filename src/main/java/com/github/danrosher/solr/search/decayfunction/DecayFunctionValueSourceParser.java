...
public abstract class DecayFunctionValueSourceParser extends ValueSourceParser {
    ...
    public ValueSource parse(FunctionQParser fp) throws SyntaxError {
        ...
        //else exception
            ...
            throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, String.format("field [%s] is of type [%s], but only numeric types are supported.",
                field, ft));
        }
    }
    ...
    protected ValueSource parseGeoVariable(FunctionQParser fp, String field, LatLonPointSpatialField ft) throws SyntaxError {

        String scaleString = fp.parseArg();
        if (scaleString == null)
            throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, "[scale] must be set for geo fields.");
    ...
    protected ValueSource parseNumericVariable(FunctionQParser fp, ValueSource vs) throws SyntaxError {
    ...
    protected ValueSource parseDateVariable(FunctionQParser fp, ValueSource vs) throws SyntaxError {
        ...
        if (scaleString == null) {
            throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, "[scale] must be set for date fields.");
        }

        originString = fp.parseArg(); // 2nd arg
        Date originDate = DateMathParser.parseMath(null,
            Optional.ofNullable(originString)
                .orElse("NOW"));
        long origin = originDate.getTime();

        long scale = DateMathParser.parseMath(originDate, "NOW"+scaleString)
            .getTime()-origin;

        if (fp.hasMoreArguments()) offsetString = fp.parseArg(); // 3rd arg
        long offset = DateMathParser.parseMath(originDate, "NOW"+offsetString)
            .getTime()-origin;
...
class GeoDecayFunctionValueSource extends ValueSource {
    ...
    public GeoDecayFunctionValueSource(double scale, double offset, double decay, DecayStrategy decayStrategy, ValueSource vs, String name) {
        this.scale = scale;
        this.offset = offset;
        this.decay = decay;
...
class NumericDecayFunctionValueSource extends ValueSource {
    ...
    public NumericDecayFunctionValueSource(double origin, double scale, double decay, double offset, DecayStrategy decayStrategy, ValueSource vs, String name) {
        this.origin = origin;
        this.scale = scale;
        this.decay = decay;
        this.offset = offset;
